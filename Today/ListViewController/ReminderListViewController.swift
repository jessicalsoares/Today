...
class ReminderListViewController: UICollectionViewController {
    ...
    var progress: CGFloat {
        let chunkSize = 1.0 / CGFloat(
            filteredReminders.count
        )
        let progress = filteredReminders.reduce(
            0.0
        ) {
    ...
    func pushDetailViewForReminder(
        withId id: Reminder.ID
    ) {
    ...
    private func makeSwipeActions(
        for indexPath: IndexPath?
    ) -> UISwipeActionsConfiguration? {
        guard let indexPath = indexPath,
              let id = dataSource.itemIdentifier(
                for: indexPath
              ) else {
            return nil
        }
